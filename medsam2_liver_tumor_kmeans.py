"""
MedSAM2 Liver & Multi-Focal Tumor Segmentation (K-Means Enhanced)
Status: "Multi-Object Mode" with Robust Statistical Detection

Logic Updates:
- Liver Detection: Largest Connected Component (Anatomy Logic)
- Tumor Detection: K-Means Clustering (Statistical Logic)
  * Separates pixels into 3 clusters: [Tumor, Healthy, Vessel]
  * Solves the "Large Tumor" problem where local adaptive thresholds fail.
...
from sklearn.cluster import KMeans  # <--- NEW: For statistical grouping
...
def process_ct_scan(ct_path, output_dir, checkpoint_path):
    ...
    # Standard Liver Window: -100 to 400 HU
    ...
    # --- A. LIVER PROMPT (Geometric Logic) ---
    ...
    # --- B. MULTI-TUMOR PROMPTS (K-Means Statistical Logic) ---
    tumor_bboxes = [] 
    ...
    if liver_bbox is not None:
        ...
        # Get valid pixels (ignore absolute black background)
        valid_mask = (liver_crop > 20)
        valid_pixels = liver_crop[valid_mask].reshape(-1, 1)
        
        if valid_pixels.size > 100:
            # === K-MEANS LOGIC START ===
            # We assume 3 tissue types in liver: 
            # 1. Dark (Tumor/Cyst)
            # 2. Medium (Healthy)
            # 3. Bright (Vessels/Calcification)
            print("   * Running K-Means Clustering on Liver Tissue...")
            kmeans = KMeans(n_clusters=3, random_state=42, n_init=10)
            labels = kmeans.fit_predict(valid_pixels)
            centers = kmeans.cluster_centers_.flatten()
            
            # Sort clusters by brightness (0=Darkest, 1=Medium, 2=Brightest)
            sorted_indices = np.argsort(centers)
            dark_idx = sorted_indices[0]
            medium_idx = sorted_indices[1]
            
            # Safety Check: Is the dark cluster actually distinct? 
            # If the "Tumor" cluster and "Healthy" cluster are too similar (e.g. < 15 intensity diff),
            # it implies the liver is uniform (no tumor).
            contrast_diff = centers[medium_idx] - centers[dark_idx]
            print(f"     > Centers: {centers[sorted_indices]} | Contrast Diff: {contrast_diff:.1f}")
            
            if contrast_diff > 15: # Threshold for "significant" darkness
                # Define dynamic threshold based on the Dark Cluster
                dark_pixels = valid_pixels[labels == dark_idx]
                # We take the max of the dark cluster as the cutoff
                dynamic_threshold = dark_pixels.max()
                
                print(f"     > Dynamic Threshold Found: < {dynamic_threshold}")
                
                # Create Mask
                tumor_candidates = (liver_crop <= dynamic_threshold) & (liver_crop > 20)
                
                # Cleanup (Morphology)
                tumor_candidates = morphology.remove_small_objects(tumor_candidates, min_size=50)
                tumor_candidates = morphology.binary_closing(tumor_candidates, morphology.disk(3))
                ...
                for idx, region in enumerate(tum_regions[:5]):
                ...
                print("     > Contrast too low. Likely healthy liver.")
            # === K-MEANS LOGIC END ===
            
        else:
             print("   | Crop contained no valid tissue")

    # Save Debug Image
    plt.figure(figsize=(12,5))
    plt.subplot(1,3,1); plt.imshow(key_slice, cmap='gray'); plt.title("Original")
    plt.subplot(1,3,2); plt.imshow(candidates, cmap='gray'); plt.title("Liver Location")
    ...
    plt.savefig(os.path.join(output_dir, "debug_kmeans_prompt.png"))
    ...
    for i in tqdm(range(len(img_windowed))):
        ...
        # Liver = Green
        if segs_liver[i].sum() > 0: 
            rgb[segs_liver[i]>0] = rgb[segs_liver[i]>0]*0.6 + LIVER_COLOR*0.4
            
        # Tumor = Red
        if segs_tumor_combined[i].sum() > 0: 
            rgb[segs_tumor_combined[i]>0] = rgb[segs_tumor_combined[i]>0]*0.4 + TUMOR_COLOR*0.6
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default='medsam2_kmeans_output')
